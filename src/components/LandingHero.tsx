...
function FlowDiagram() {
  const steps = [
    {
      icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" stroke="#6366f1" strokeWidth="2" strokeLinecap="round"/><circle cx="12" cy="7" r="4" stroke="#6366f1" strokeWidth="2"/></svg>),
      label: "Your Wallet", sub: "Any Ethereum wallet",
    },
    {
      icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 2L3 7v5c0 5.25 3.75 10.15 9 11.35C17.25 22.15 21 17.25 21 12V7L12 2z" fill="#6366f1"/></svg>),
      label: "SentinelLP Agent", sub: "Monitors every 5 min",
    },
    {
      icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="#6366f1" strokeWidth="2"/><path d="M12 6v6l4 2" stroke="#6366f1" strokeWidth="2" strokeLinecap="round"/></svg>),
      label: "AI Decision Engine", sub: "Economic reasoning", highlight: true,
    },
    {
      icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="#00D395"/><path d="M8 12l3 3 5-6" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>),
      label: "KeeperHub", sub: "Gas-sponsored execution", highlight: true,
    },
    {
      icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><rect x="2" y="7" width="20" height="14" rx="2" stroke="#6366f1" strokeWidth="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2" stroke="#6366f1" strokeWidth="2"/></svg>),
      label: "Ethereum", sub: "Onchain confirmation",
    },
    {
      icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.63A2 2 0 012 .18h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.91a16 16 0 006.18 6.18l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0121.99 15l-.07 1.92z" stroke="#6366f1" strokeWidth="2" strokeLinecap="round"/></svg>),
      label: "You're Notified", sub: "Telegram + audit trail",
    },
  ];
          ...
          {steps.map((step, i) => (
            <div key={i} className="hero-flow-step">
              <div className="hero-flow-step-left">
                <div className={`hero-flow-icon ${step.highlight ? "highlight" : ""}`}>{step.icon}</div>
              ...
              {step.highlight && (
                <div className="hero-flow-badge">
                  {step.label === "AI Decision Engine" ? "Decides" : "Executes"}
                </div>
              )}
            </div>
          ))}
    ...
    <div className="landing">
        ...
        <div className="hero-inner">
          <div className="hero-left">
            ...
            <FadeIn delay={240} className="hero-cta-wrap">
              ...
              <p className="hero-terms">No private keys required · Non-custodial · 0.05% per rebalance</p>
            </FadeIn>
      ...
      <section className="section" id="how-it-works">
        <FadeIn>
          ...
          <p className="section-subtitle">SentinelLP handles the full rebalance cycle automatically. You set it up once and never think about it again.</p>
        </FadeIn>
      ...
      <section className="section section-gray" id="features">
        <FadeIn>
          ...
          <p className="section-subtitle">Every component is chosen for reliability, not novelty.</p>
        </FadeIn>
        <div className="features-grid">
          {[
            ...
            { tag: "Observability", title: "Full Audit Trail", desc: "Every decision the agent makes is logged — timestamp, reasoning, action, KeeperHub job ID, and transaction hash.", icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" stroke="#6366f1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg> },
            { tag: "Performance", title: "Tick-Aware Rebalancing", desc: "The agent proposes new tick ranges snapped to the pool's tick spacing. It understands fee tiers — 0.05% pools use 10-tick spacing, 0.3% use 60, 1% use 200.", icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke="#6366f1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg> },
          ].map((f, i) => (
      ...
      <section className="section section-gray" id="pricing">
        <FadeIn>
          ...
          <p className="section-subtitle">We only charge when we rebalance. No monthly fees, no subscriptions.</p>
        </FadeIn>
        <FadeIn delay={80}>
          <div className="pricing-card">
            <div className="pricing-left">
              ...
              <p className="pricing-desc">Charged only when SentinelLP rebalances your position. If it's not worth rebalancing, we don't charge.</p>
              <ConnectButton.Custom>
                {({ openConnectModal }) => (
                  <button className="btn-hero" onClick={openConnectModal} style={{ marginTop: 24 }}>Start for free</button>
                )}
              </ConnectButton.Custom>
            </div>
            <div className="pricing-right">
              {[
                ...
                "AI-powered decision making on every rebalance",
                "Chainlink real-time price feeds",
                "Non-custodial — revoke anytime",
                "Free to register — pay only on rebalance",
              ].map((item, i) => (
                <div key={i} className="pricing-feature">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4L19 7" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  {item}
                </div>
              ))}
            </div>
      ...
      <section className="section" id="faq">
        ...
        <div className="faq-grid">
          {[
            { q: "Does SentinelLP hold my funds?", a: "Never. SentinelLP uses Uniswap's native operator approval (setApprovalForAll) which allows it to manage your positions without ever holding your tokens. All tokens go directly from the pool back to your wallet." },
            { q: "How does gas sponsorship work?", a: "KeeperHub sponsors gas for rebalance transactions on Sepolia testnet. On mainnet, gas handling depends on your KeeperHub configuration. You never need ETH in your wallet specifically for gas." },
            { q: "How does the AI decide when to rebalance?", a: "Our AI compares the estimated daily fee loss from being out of range against the gas cost of rebalancing. If the economics justify it — it rebalances. Otherwise, it waits. All decisions are logged." },
            { q: "Which Uniswap v3 pools are supported?", a: "SentinelLP supports all Uniswap v3 pools on Ethereum — any fee tier (0.05%, 0.3%, 1%) and any token pair. The tick spacing is automatically calculated per fee tier." },
            { q: "Can I stop the agent?", a: "Yes, anytime. Click Deregister in the dashboard to stop monitoring. You can also revoke operator approval directly from Uniswap's interface." },
            { q: "What network is this on?", a: "Currently on Ethereum Sepolia testnet for beta. Mainnet launch is targeted for the KeeperHub hackathon submission date (August 13, 2026)." },
          ].map((item, i) => (
      ...
      <section className="section footer-cta-section">
        <FadeIn>
          <h2 className="footer-cta-title">Start protecting your positions today.</h2>
          <p className="footer-cta-sub">Connect your wallet and be up and running in under 5 minutes.</p>
          <ConnectButton.Custom>
            {({ openConnectModal }) => (
              <button className="btn-hero" onClick={openConnectModal}>Connect Wallet</button>
            )}
          </ConnectButton.Custom>
        </FadeIn>
      </section>
